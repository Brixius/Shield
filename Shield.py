...
def moveBall(ball, randspeed):
    ball.x += (1 + randspeed)
    return ball

#Checks to see if a point has been scored returns new score
def checkPointScored(hole1, ball, score, subnum, blocknum, trialnum, randspeed, gametimer, blockend, L1, L2, L3, L4, L5, L6):
  ...
  if ball.right >=(WINDOWWIDTH - LINETHICKNESS):
    ...
    if hole1.left <= ball.right and hole1.top < ball.top and hole1.bottom > ball.bottom:
        score += 1
        hit = 1
    elif ball.right >= (WINDOWWIDTH - LINETHICKNESS) and score >= 0:
        score -= 1
        if score < 0:
            score = 0
        hit = 0
    ...
    return score,randspeed,ball.x,ball.y,blocknum,trialnum,gametimer,blockend
  else: return score,randspeed,ball.x,ball.y,blocknum,trialnum,gametimer,blockend
...
def quitwrite():
	eyetracker.close()
	csv_out = open("./Logs/" + str(subnum) + "ShieldGameLogLR.csv",'wb')
	mywriter = csv.writer(csv_out)
	for row in zip(L1, L2, L3, L4, L5, L6):
		mywriter.writerow(row)
	csv_out.close()
	csv_out2 = open("./Logs/" + str(subnum) + "ShieldReadLogLR.csv",'wb')
	mywriter2 = csv.writer(csv_out2)
	for row in zip(R1, R2, R3, R4, R5):
		mywriter2.writerow(row)
	csv_out2.close()
	csv_out3 = open("./Logs/" + str(subnum) + "ShieldAnsLogLR.csv",'wb')
...
#            blocknum += 1
        ...
        ball = moveBall(ball, randspeed)
        score, randspeed, ball.x, ball.y, blocknum, trialnum, gametimer, blockend = checkPointScored(hole1, ball, score, subnum, blocknum, trialnum, randspeed, gametimer, blockend, L1, L2, L3, L4, L5, L6)
